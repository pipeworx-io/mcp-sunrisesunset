...
`tools/list` at `https://gateway.pipeworx.io/sunrisesunset/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_times \
  -H 'Content-Type: application/json' \
  -d '{"lat":40.7128,"lng":-74.006}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_times`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "sunrisesunset": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-sunrisesunset"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-sunrisesunset
```
